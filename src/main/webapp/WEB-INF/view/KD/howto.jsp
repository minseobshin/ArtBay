<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Insert title here</title>
<link rel='stylesheet' type='text/css' href="./css/application.css">
<link rel='stylesheet' type='text/css' href="./css/basic.css">
<link rel="stylesheet" href="http://fonts.googleapis.com/css?family=Open+Sans:400,300,700,800">
<link rel="stylesheet" href="css/reset.css">
<link rel="stylesheet" href="css/index.css">
<script src="https://code.jquery.com/jquery-3.6.0.js"></script>
</head>
<body>
<!-- 메뉴 네비게이션 -->
	<%@include file="../header.jsp" %>
	
<div class="ka-container ">
        <!-- START CONTENT WRAPPER -->
        <div class="ka-content-wrapper">
            <!-- START CONTENT SECTION -->
            <div class="content">
<div class = "howtoform">
		<h1>경매 소개</h1>
</div>

<div class="container">
    <div id="Howto">
        <div class="howtobtn" style="margin-top: 0px;">
            <ul>
                <li class="">
                    <a href="howto">경매 소개</a>
                    <div class="howto-subbtn" style="">
                        <ul class="a1">
                            <li class=""><a href="#top">온라인 경매</a></li>
                            <li class=""><a href="#bottom">온라인 경매</a></li>
                        </ul>
                    </div>
                </li>
                <li>
                    <a href="/ConsignGuide">응찰 안내</a>
                    <div class="howto-subbtn" style="display: none;">
                        <ul class="a2" style="display: block;">
                            <li><a href="#">회원 가입</a></li>
                            <li><a href="#">프리뷰</a></li>
                            <li><a href="#">응찰</a></li>
                            <li><a href="#">낙찰 결과 확인 및 결제</a></li>
                            <li><a href="#">배송, 설치 및 보관</a></li>
                        </ul>
                    </div>
                </li>
                <li>
                    <a href="#">위탁 안내</a>
                    <div class="howto-subbtn" style="display: none;">
                        <ul class="a3" style="display: block;">
                            <li><a href="#">Why Art Bay?</a></li>
                            <li><a href="#">위탁 절차</a></li>
                        </ul>
                    </div>
                </li>
            </ul>
        </div>
        <div class="howto-desc" style="margin-top: 0px;">
            <div class="auc-intro">
            <div id="top">
                    <h2>온라인 경매</h2>
                    <p>품었기 자신과 옷을 힘있다. 작고 우리 행복스럽고 듣는다. 쓸쓸한 이는 되려니와, 길을 아니더면, 것이다. 사랑의 그와 수 불러 구하지 커다란 꽃 있다.</p>
                    <div>
                        <div class="auc-img01">
                            <img src="/img/img1.png" >
                        </div>
                        <div class="auc-desc">
                            <h4>
                                경매
                                <img src="/img/online-ico@1x.png" >
                            </h4>
                            <p>
                                관현악이며, 얼음 보이는 인생을 약동하다. 영락과 청춘 청춘의 그들은 물방아 되려니와, 너의 것이다. 그들은 소금이라 하는 청춘 구하기 얼음이 발휘하기 위하여서.
                            </p>
                        </div>
                    </div>
                </div>
                <div id="bottom">
                    <h2>온라인 경매</h2>
                    <p>
                        온라인 경매는 아마추어의 미술품 경매와 합리적인 가격대의 작품을 선보입니다.<br>홈페이지에서 응찰이 가능하며 시공간 제약 없이 누구나 쉽게 미술품 경매에 참여할 수 있습니다.
                    </p>
                    
                    
                    <div>
                        <div>
                            <div class="img">
                                <img src="/img/aucimg.jpg">
                            </div>
                            <div>
                                <h4>
                                    온라인경매
                                    <img src="/img/online-ico@1x.png">
                                </h4>
                                <p class="desc">
                                    봄바람을 뭇 가치를 것이다. 천고에 청춘에서만 청춘이 사람은 더운지라 피에 장식하는 보내는 황금시대의 봄바람이다. 청춘의 돋고, 피에 아니더면, 이성은 있으며, 불어 힘차게 품으며, 그리하였는가? 눈에 든 따뜻한 미묘한 수 아니더면, 그들에게 운다. 아름답고 인간은 밥을 피에 방황하였으며, 풀밭에 군영과 부패뿐이다. 같으며, 피가 풀이 황금시대의 과실이 내는 말이다. 그들은 같은 이상, 살 하는 커다란 능히 칼이다. 황금시대를 주는 무엇이 아니다. 창공에 용감하고 기관과 석가는 우리의 쓸쓸하랴?
                                </p>
                                <p class="date">
                                    매주 월요일 시작 ~ 금주 금요일 마감 (1주 간격)
                                </p>
                                    <p class="check">
                                        <a href="list">진행 중인 경매의 출품작 확인하기 →</a>
                                    </p>
                            </div>
                        </div>
                        <div>
                            <div class="img">
                                <img src="/img/aucimg.jpg">
                            </div>
                            <div>
                                <h4>
                                    온라인경매 결과
                                    <img src="/img/online-ico@1x.png" >
                                </h4>
                                <p class="desc">
                                    타오르고 피가 품으며, 능히 이상의 가는 이상의 끓는 것이다. 앞이 산야에 물방아 아니한 들어 아니다. 가치를 꾸며 소담스러운 불어 끝에 피에 기쁘며, 이상 같지 보라. 풀이 열락의 소금이라 있다. 천지는 끝까지 충분히 얼마나 봄날의 것이다. 이성은 커다란 열락의 긴지라 가는 하는 행복스럽고 봄바람이다. 이 피어나는 뜨고, 아니다. 피부가 보이는 물방아 청춘의 뛰노는 귀는 봄바람이다. 있는 이상은 스며들어 물방아 이상의 눈이 사막이다. 가슴이 원대하고, 길을 있는 웅대한 있는 산야에 부패뿐이다. 예가 가슴이 얼음과 청춘 가치를 모래뿐일 같은 아니다.
                                </p>
                                <p class="date">
                                    매주 월요일 시작 ~ 금주 일요일 마감 (1주 간격) 결과
                                </p>
                                    <p class="check">
                                        <a href="Result">경매의 출품작 결과 확인하기 →</a>
                                    </p>
                            </div>
                        </div>
                    </div>
                </div>
                
                <div id="sns">
                    <p class="sns-tit">
                        <img src="/img/cal-ico@1x.png" alt="경매 일정 아이콘" >
                        경매 일정 확인?
                    </p>
                    <p class="sns-desc">
                        공자는 열매를 수 할지라도 그들은 것이다. 넣는 들어 커다란 온갖 품에 따뜻한 때문이다.
                    </p>
                    <ul>
                        <li class="active">
                            <img src="/img/instagram.png" alt="인스타그램">
                            <a href="#">인스타그램</a>
                        </li>
                        <li>
                            <img src="/img/blog.png" alt="네이버 블로그">
                            <a href="#">네이버 블로그</a>
                        </li>
                        <li>
                            <img src="/img/plusfriendlogo.png" alt="카카오 플러스친구">
                            <a href="#">카카오 플러스친구</a>
                        </li>
                        <li>
                            <img src="/img/facebook.png" alt="페이스북 페이지">
                            <a href="#">페이스북 페이지</a>
                        </li>
                    </ul>
                </div>
            </div>
        </div>
    </div>
</div>


	        </div>
        </div>
    </div>
    <!-- 메인 푸터 -->
	<%@include file="../footer.jsp" %>
</body>
</html>